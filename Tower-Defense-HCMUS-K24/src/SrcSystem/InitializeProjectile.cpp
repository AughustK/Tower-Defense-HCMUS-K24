...
EntityID ProjectilePoolSystem::spawn(World& world,
    ...
    cout << "[ProjectilePoolSystem] Spawning projectile" << e << "\n";
...
void ProjectilePoolSystem::clearPool(World& world)
{
    for (EntityID e : pool)
    {
        // only hide ones that still have a sprite
        if (world.hasComponent<SpriteComponent>(e)) {
            hideUsedProj(world, e);
        }
    }
    nextIndex = 0;
}
...
void ProjectilePoolSystem::hideUsedProj(World& world, EntityID en) 
{
    ...
    world.getSystem<SpriteRenderSystem>()->hideEntity(en);
